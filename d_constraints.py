...
def battery_and_acc_constraint(velocity_profile, dt, cum_d_array, slope_array, lattitude_array, longitude_array, cum_d, i, InitialBatteryCapacity, timeoffset, FinalBatteryCapacity):
    '''
    ...
    P_total, _ = calculate_power_req(avg_speed, acceleration, slope_array)
    ...
    energy_consumed = ((P_total - P_solar) * dt).cumsum() / HR # Wh
    ...
    return np.min(battery_profile), (BATTERY_CAPACITY - SAFE_BATTERY_LEVEL) - np.max(battery_profile), MAX_P - np.max(P_total), final_battery_lev # Ensure battery level bounds
