...
def battery_and_acc_constraint(velocity_profile, dt, cum_d_array, slope_array, lattitude_array, longitude_array):
    ...
    return np.min(battery_profile), np.max((P_resistance.clip(0) / (CAR_MASS * avg_speed)) - acceleration) # Ensure battery level never falls below safe level and acceleration ??
